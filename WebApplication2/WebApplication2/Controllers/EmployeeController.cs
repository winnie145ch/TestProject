...
namespace WebApplication2.Controllers
{
    ...
    public class EmployeeController : ApiControllerBase
    {
        ...
        [Route("Update")]
        [HttpPost]
        public IActionResult Edit([FromBody] Employee edit)
        {
            try
            {
                if (string.IsNullOrEmpty(edit.EmpNo))
                    _context.Employee.Add(edit);
                else
                {
                    var check = _context.Employee.Where(x => x.EmpNo == edit.EmpNo);
                    if (check == null)
                        return Ok("查無此人");
                    _context.Employee.Update(edit);
                }
                _context.SaveChanges();
                return Ok("沒有改變");
                ...
                //    var updateStr = _context.Employee.Where(x => x.EmpNo == ).AsQueryable();
